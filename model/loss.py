...
def compute_loss(
    ...
    batch_size = tf.cast(
        len(y_true), tf.dtype(tf.float32)
    )  # ensure to be float so that the resultant loss is float
    ...
    for l, grid_size in zip(range(num_layers), GRID_SIZES):
        ...
        grid = np.array(
            [
                [[float(x), float(y)]] * num_bounding_boxes
        ...
        pred_boxes = tf.reshape(
            y_pred[..., 3:], (-1, grid_w * grid_h, num_bounding_boxes, 5)
        )
        true_boxes = tf.reshape(
            y_true[..., 3:], (-1, grid_w * grid_h, num_bounding_boxes, 5)
        )
        ...
        xy_loss = tf.sum(
            tf.sum(tf.square(y_true_xy - y_pred_xy), axis=-1) * y_true_conf, axis=-1
        )
        ...
        wh_loss = tf.sum(
            tf.sum(tf.square(tf.sqrt(y_true_wh) - tf.sqrt(y_pred_wh)), axis=-1)
            * y_true_conf,
            axis=-1,
        )
        ...
        conf_loss = tf.sum(
            tf.square(true_confidence * iou - pred_confidence) * true_confidence,
            axis=-1,
        )
