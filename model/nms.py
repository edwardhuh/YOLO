...
for i in range(5):
    random_jitter = np.random.normal(0, 0.0025, len(bounding_boxes) * 4).reshape(
        (len(bounding_boxes), 4)
    )
    noised_box = bounding_boxes + random_jitter
    noised_boxes.append(noised_box)

visualize(
    filepath=y[0]["file"], bounding_boxes=y[0]["boxes"][0], found_boxes=noised_boxes
)
